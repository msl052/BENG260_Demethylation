...
for klok=1:klokmax
  ...
  gE=gNa*ENa+gK*EK+gLbar*EL;         %gE=g*E  (current density)
  
  ihh(klok)=gNa*(v-ENa)+gK*(v-EK)+gLbar*(v-EL) % Paper
  im(klok) = ihh(klok)*pi*d*NL/dx+(dx-NL)/dx*v*gm;
  %save old value of v for checking purposes:
  v_old=v;
  %update v:
  %v=(v+(dt/C)*(gE+pNodal(klok)))/(1+(dt/C)*g);
...
time = (1:klokmax).*dt;
plot(time,im);
ylabel('current density [muA/cm]');
%save('NodalCurrentDensity','ihh')
