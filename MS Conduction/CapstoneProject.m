...
ismyelin = [true, true, true, true, false, false, true];
myelinlen = [NL, NL, NL, NL, 400, 0, 0 ,NL];
for i = 1:length(ismyelin)
    ...
    for klok=1:klokmax
      ...
      if ismyelin == false
        im(klok) = gm*v/myelinlen(i); %current density (muA/cm)
      else
        ihh(klok)=gNa*(v-ENa)+gK*(v-EK)+gLbar*(v-EL); % muA/cm^2
        im(klok) = (ihh(klok)*pi*d*NL/dx+(dx-NL)/dx*v*gm)/myelinlen(i); % muA/cm
        %im(klok) = ihh(klok)
      end
